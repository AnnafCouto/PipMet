...
#' @param mat A normalized matrix generated by 'PipMet::normalize()'
#' @return A list comprising a matrix with the normalized intensities for each spectrum produced and the path to the volcano plot folder.
...
#' }
#'
vol_lvl1 <- function(n, mat, metadata, myDir) {
  setwd("Statistics")
  if(!dir.exists('Volcanos')==TRUE) {dir.create('Volcanos')}
  setwd("Volcanos")

  volDir <- getwd()
  x <- menu(names(metadata), graphics = TRUE, title = "Choose a condition to compare") # ask condition to compare from the metadata table
  opt <- select.list(unique(metadata[, as.integer(x)]), preselect = NULL, multiple = TRUE, title = "Select two characteristic to compare", graphics = TRUE) # from that condition, which characteristics to compare
