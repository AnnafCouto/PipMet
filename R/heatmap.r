...
#' }
#' }
heatmap <- function(n, metadata, myDir, colors, pic_extension = c('.tiff', '.png'), replicate = NULL, sample_names = NULL) {
  ...
  if (replicate == TRUE | replicate %in% colnames(metadata)) {
    ...
    mat_ident <- mat[-which(is.na(n[, 3])), ]
    nhmedia_ident <- matrix(nrow = nrow(mat_ident), ncol = length(unique(metadata [,replicate])))
