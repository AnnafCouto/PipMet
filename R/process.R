...
process <- function(raw_data, metadata, myDir, colors, peakMonitor = FALSE, ions = NULL,
    pictures = TRUE, example = FALSE, filter = NULL, pic_extension = c(".tiff",
        ".png"), group = NULL) {

    message(paste0("Processing ", length(raw_data$files), " files..."))
    # peak picking
    message(paste0("Detecting peaks..."))
    mfp <- MatchedFilterParam(fwhm = 5, binSize = 0.5, steps = 2, mzdiff = 0.5,
        snthresh = 2, max = 500)
    ...
    if (example == FALSE) {
        if (!is.null(filter)) {
            ...
            xdata <- refineChromPeaks(xdata, param = FilterIntensityParam(threshold = as.integer(filter),
    ...
    if (nrow(xdata@phenoData@data) > 1) {
        ...
        xdata3 <- groupChromPeaks(xdata2, param = PeakDensityParam(sampleGroups = metadata[,
            group], bw = 0.5, minSamples = 1, maxFeatures = 500, minFraction = 0.4))
        if (!example == TRUE) {
        ...
        if (exists("xdata3") & pictures == TRUE) {
            ...
            if (".tiff" %in% pic_extension) {
                tiff("plotChromPeakImage.tiff", units = "cm", width = 16, height = 16,
                  res = 900, bg = "NA")
            ...
            if (".png" %in% pic_extension) {
                png("plotChromPeakImage.png", units = "cm", width = 16, height = 16,
                  res = 900, bg = "NA")
                par(mar = c(5, 9, 4, 1) + 0.1)
                plotChromPeakImage(xdata3)
                dev.off()
            ...
            for (i in seq_len(length(colors))) {
                ...
                if (".tiff" %in% pic_extension) {
                  ...
                  legend("right", inset = c(-0.23, 0), legend = names(colors[[i]][[2]]),
            ...
            for (i in seq_len(length(colors))) {
                ...
                if (".tiff" %in% pic_extension) {
                  ...
                  legend("topright", legend = names(colors[[i]][[2]]), col = colors[[i]][[2]],
                    fill = colors[[i]][[2]], box.lty = 0, cex = 1, bg = "transparent")
                  plotAdjustedRtime(xdata3, col = colors[[i]][[2]][colors[[i]][[1]]])
                  legend("bottomright", legend = names(colors[[i]][[2]]), col = colors[[i]][[2]],
                    fill = colors[[i]][[2]], box.lty = 0, cex = 1, bg = "transparent")
                  dev.off()
                }
                # png
            ...
            if (peakMonitor == TRUE | peakMonitor == "Yes") {
                ...
                for (ii in seq_len(length(ions))) {
                  crom <- chromatogram(xdata3, rt = c(ions[[ii]][["rt"]] - 5,
                    ions[[ii]][["rt"]] + 5), mz = ions[[ii]][["mz"]], include = "none")
                  for (i in seq_len(length(colors))) {
                    # tiff
                    if (".tiff" %in% pic_extension) {
