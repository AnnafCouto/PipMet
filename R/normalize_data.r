  ...
  while (okay == 2) {
    ...
    if (nrow(quant) > 1 & nrow(metadata) > 1) {
      ...
      if (nrow(mat) > 50) {
        ...
        try(normalyzer(jobName = "Normalyzer_results", designPath = designFp, dataPath = dataFp, outputDir = myDir, sampleColName = "sample", groupColName = group, requireReplicates = FALSE, sampleAbundThres = nrow(mat)))
