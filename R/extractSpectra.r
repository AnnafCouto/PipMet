    ...
    if (is(x, "xsAnnotate")) {
        if (sum(grepl("X", colnames(x@groupInfo)) == TRUE) == 0) {
            ...
            for (j in seq_along(rownames(x@xcmsSet@phenoData))) {
                temp[[j]] <- grep(paste0("^X", rownames(x@xcmsSet@phenoData)[j],
        ...
        for (i in seq_along(x@pspectra)) {
            ...
            if (length(x@pspectra[[i]]) > 1) {
                ...
                # work.
                if (sum(grepl("X", colnames(x@groupInfo)) == TRUE) == 0) {
                  spc <- cbind(x@groupInfo[x@pspectra[[i]], "mz"], x@groupInfo[x@pspectra[[i]],
                    sv])
                } else {
                  spc <- cbind(x@groupInfo[x@pspectra[[i]], "mz"], rowMax(x@groupInfo[x@pspectra[[i]],
                    sv]))
                }
                if (sum(is.na(spc)) >= 1) {
                ...
                spc <- cbind(x@groupInfo[x@pspectra[[i]], "mz"], max(x@groupInfo[x@pspectra[[i]],
                  sv], na.rm = TRUE))
            }
            pseudospeclist[[i]] <- new("pseudospectrum", id = i, rt = median(x@groupInfo[x@pspectra[[i]],
                "rt"]), spectrum = spc)
        }
    }
