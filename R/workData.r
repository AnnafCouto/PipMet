...
workData <- function(myDir = NULL, sample_dir = NULL, metadata = NULL, extension = NULL,
    ...
    if (nrow(metadata) > 1 & nrow(pre_anno) > 1) {
        ...
        if (pictures == TRUE & nrow(metadata) > 1) {
            pic <- dlg_list(c("Volcano - level 1", "Volcano - level 2", "PCA - All spectra",
                "PCA - Identified spectra only", "Heatmaps", "All", "None"),
                multiple = TRUE, title = "Statistics pictures:")$res
            # plot volcanos
            okay <- 1
            if ("Volcano - level 1" %in% pic | "All" %in% pic) {
                # Volcano level 2 can only be made if volcano - level 1 is
                # too
                while (okay == 1) {
            ...
            # plot PCA
            if ("PCA - All spectra" %in% pic | "All" %in% pic) {
                quiet(PCA_general(n, metadata, myDir, colors, pic_extension = pic_extension,
                  example = example))
            }
            if ("PCA - Identified spectra only" %in% pic | "All" %in% pic) {
                quiet(PCA_identified(n, metadata, myDir, colors, pic_extension = pic_extension,
                  example = example))
            }
            ...
            if ("Heatmaps" %in% pic | "All" %in% pic) {
                quiet(heatmap(n, metadata, myDir, colors, pic_extension = pic_extension,
                  replicate = replicate, sample_names = NULL))
            }
        }
    }
    ...
    if (is.null(lib_build)) {
        lib_build <- dlg_message("Would you like to create a in-house database with the identified spectra?",
            type = "yesno")$res
    }
