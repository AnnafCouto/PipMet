...
Before the processing starts, the user must point what kind of parallelization he must apply to the process (Figure 1). For more information, read on the the BiocParallel package.


```{r, out.width='25%', fig.align='center', fig.cap='Figure 1. Parallelization mode.', echo=FALSE}
knitr::include_graphics('02.png')
``` 


Furthermore, the user may inform m/z and rt (retentions time) for specific ions to be monitored throuhout the entire processing. For example, internal standards can be monitored through the 'Monitor EIC' option (Figure 2)


```{r, out.width='25%', fig.align='center', fig.cap='Figure 2. Peak monitoring.', echo=FALSE}
knitr::include_graphics('02.png')
``` 


As a first step, the user will be asked to point the folder where the files to be processed are located (Figure 3). Once we set `example = TRUE`, this pop-up will be supressed.


```{r, out.width='40%', fig.align='center', fig.cap='Figure 3. Samples folder.', echo=FALSE}
knitr::include_graphics('1.png')
``` 


Following that, the user is asked to provide a name for the processing project, which will be used as the name of the folder where the processing will occur and create the output files (Figure 4). For the example, the project will be named 'Example'.
...
Here, the user may create different categories to describe the samples with column names (Figure 7). It is important to make sure there is no other filled columns or rows except those containing the samples. For that, select the empty columns and rows and delete its content before saving. If there is the content, the software will alert the user to delete and reupload the metadata file.


```{r, out.width='50%', fig.align='center', fig.cap='Figure 7. Metadata information.', echo=FALSE}
knitr::include_graphics('5.png')
``` 

When the files are properly read, a 'Visualization_results' folder will be created in your project folder the software saves generated pictures for the user to evaluate the quality of the initial data, such as total ion count and base peak chromatograms and heatmaps (Figure 8).


```{r, out.width='40%', fig.align='center', fig.cap='Figure 8. Chromatograms and cluster of samples.', echo=FALSE, fig.show='hold'}
...
The first step in the preprocessing phase is the selection of peaks among noise, done with the MatchedFilter algorythm from xcms. Therefore, the true peaks will be picked as the one that can pass gaussian filters. There is also de possibility of applying a filter to select only peaks above a certain intensity threshold (Figure 9).In the present example, we will be using a 150 intensity threshold for final spectra quality improvement.


```{r, out.width='25%', fig.align='center', fig.cap='Figure 9. Intensity filter and threshold.', echo=FALSE,  fig.show='hold'}
...
```{r, out.width='80%', fig.align='center', fig.cap='Figure 10. Chromatograms postprocessing.', echo=FALSE}
knitr::include_graphics('group_postprocessedChromatogram.png')
``` 


```{r, out.width='40%', fig.align='center', fig.cap='Figure 11. Boxplot of intensities postprocessing and instensity of detected peak in postprocessed data.', echo=FALSE, fig.show='hold'}
...
```{r, echo=FALSE, results='asis', fig.cap='Table 2. Annotation file.'}
...
For improving the identification, the retention index can also be used, if a RI.csv file is provided, containing a 'rt' and 'RI' column (Figure 14). Once the example files are of a slim range of retention time, we will not add any retention index.


```{r, out.width='30%', fig.align='center', fig.cap='Figure 14. Retention index.', echo=FALSE}
knitr::include_graphics('12.png')
``` 


At the end of this substep, some pictures are plotted, such as hierarchycal clustering and a heatmaps based on the similarity of the samples (Figure 15 and 16). For more information, check the CluMSID R package ^5^.


```{r, out.width='100%', fig.align='center', fig.cap='Figure 15. Hierarchycal plot.', echo=FALSE}
knitr::include_graphics('hierarchy_plot.png')
``` 


```{r, out.width='100%', fig.align='center', fig.cap='Figure 16. Heatmap.', echo=FALSE}
...
The PipMet package, through the NormalyzerDE package, offers different methods of normalization so that the user can choose the best to addapt to the data after evaluation the models and the printed report. For the normalization to occur, the user must point the category from which the samples are better grouped (Figure 18). For the example, select 'group'.



```{r, out.width='50%', fig.align='center', fig.cap='Figure 18. Groups for normalization.', echo=FALSE}
knitr::include_graphics('7.png')
``` 



The normalyzerDE R package provides different normalization methods and a report with metrics for the user to pick the best normalization that fit his data (Figure 19). In the Example, a folder 'Normalyzer_results' is created and the user is invited to look at the pdf report and to choose among the methodologies. Here, we picked the CycLoess normalization. Then a histogram of the variance and standard deviation is plot in the 'Statistics' folder, along with a boxplot of the same metrics and its outliers. 



```{r, out.width='40%', fig.align='center', fig.cap='Figure 19. Boxplot and histogram of variance and standard deviation.', echo=FALSE,fig.show='hold'}
par(mfrow = c(1,2))
...
The variance and standard deviation is calculated from the intensities of the representative ion and the second most intense in each of the spectra. If the spectra is not correctly proposed (for example, if it has a coelution not detected), the rate between the representative peak and the second most intense will not be stable along the samples where the compound appears, showing too much variance and standard deviation. The same may happen to bleeders or contaminants. Therefore, the user must monitor those results in order to have variance and standard deviation (sd) as low as possible. If some compound present high variance or sd, the user may choose to remove outliers in general (shown in the boxplot image) ou remove compounds with variance bigger than a threshold (Figure 20). For the example, the CycLoess normalization is enough and it won't be necessary to remove outliers ou any compound.



```{r, out.width='30%', fig.align='center', fig.cap='Figure 20. Normalization check.', echo=FALSE, fig.show='hold'}
knitr::include_graphics('13.png')
knitr::include_graphics('14.png')
``` 


Further on, the user is asked which column from the metadata table represents the replicate column, if there is any (Figure 21). For the example files, that information is also represented by the 'group' column.



```{r, out.width='25%', fig.align='center', fig.cap='Figure 21. Replicate information.', echo=FALSE}
...
The first is volcano. The user is instructed to select a condition from the metadata table to compare two characteristics from that condition. Here, we choose 'group' to compare the 'Energy' and 'Sugar' compounds (Figure 22).



```{r, fig.align='center', out.width='40%', fig.cap='Figure 22. Volcano setup.', echo=FALSE, fig.show='hold'}
knitr::include_graphics('16.png')
``` 



If more than one category to describe the samples are available, the user may do a Level 2 volcano plot, to compare using to describers. Such as 'Energy: Control X Treatment' and 'Sugar: Control X Treatment'. 

Along with each of the comparison for plotting the Volcano plot, a file containing the p-value (significance) and fold change of the comparisons is written inside the volcano plot folder (Figure 23). Next, the PCA of all samples are plotted, using all spectra proposed (Figure 24). 


```{r, out.width='40%', fig.align='center', fig.cap='Figure 23. Volcano plot.', echo=FALSE}
knitr::include_graphics('volcano_identified.png')
``` 


```{r, out.width='40%', fig.align='center', fig.cap='Figure 24. PCA of samples.', echo=FALSE}
knitr::include_graphics('PCA_named_sample.png')
``` 


The user is then asked to choose a particular condition to plot the PCA for specified samples only (Fgiure 25). For example, we choose to only look at Sugarcane and Energycane samples, therefore, we choose 'group' and select both 'Sugar' and 'Energy' describers.



```{r, out.width='25%', fig.align='center', fig.cap='Figure 25. PCA selected samples.', echo=FALSE, fig.show='hold'}
knitr::include_graphics('PCAselected_named_group.png') 
``` 


The PCA is plotted again only for the identified compounds, repeating the same process as before: plot of all samples and asking for specific samples (Figure 26).


```{r, out.width='50%', fig.align='center', fig.cap='Figure 26. PCA of all samples and selected samples - Identified compounds only.', echo=FALSE, fig.show='hold'}
knitr::include_graphics('PCA_named_group_identified.png') 
knitr::include_graphics('PCAselected_named_group_identified.png') 
```


The heatmaps are plotted colored by the different categories describing the samples and to plot them, the user is asked how the samples should be named if the pictures (Figure 27 and 28).


```{r, out.width='40%', fig.align='center', echo=FALSE, fig.cap='Figure 27. Heatmaps of all spectra generated and heatmap of only identified spectra.',fig.show='hold'}
knitr::include_graphics('group_heatmap_scaled_geral.png')
knitr::include_graphics('heatmap_scaled_ident_group_.png')
``` 


```{r, out.width='60%', fig.align='center', echo=FALSE, fig.cap='Figure 28. Heatmap of identified spectra with samples as the mean of their replicates.'}
...
The pop-ups only appear if the user do not specify the information required in the parameters of the main function (Figure 29). If everything is given prior to the beginning of the processing, the function will work without any intervention or pop-up. For the parameters, refer to the `PipMet` documentation.


```{r, out.width='40%', fig.align='center', echo=FALSE, fig.cap='Figure 29. Processing done.'}
...
Once the processing is done, the identified compounds can be used to create a in-house database to be uploaded in NIST MS Search Software (Figure 30). In this case, the user is asked whether to create or not, as following:


```{r, out.width='40%', fig.align='center', echo=FALSE, fig.cap='Figure 30. In-house database creation.'}
knitr::include_graphics('22.png')
```


For this extra step, the user may provide more information than the name, retention time and index and spectrum (Figure 31). A dataInfo.csv will be created in the folder, if the user agree to add more information, where information about molecule describers such as CAS, InChiKey, PubChem ID, ChemSpider ID, retention index and more can be included. 



```{r, out.width='30%', fig.align='center', echo=FALSE, fig.cap='Figure 31. Add more information to the compounds.', fig.show='hold'}
...
knitr::kable(read.csv(system.file("extdata", "dataInfo.csv", package = "PipMet")), caption ='Table 3. DataInfo table.' )
```


After filling the forms and pressing 'Ok', the software will still try to recover the missing information and more. It will be asked if the user would like to add retention index information, if there missing indexes (Figure 32). For that, the user must provide the kind of retention index (supported one are 'linear', 'lee' and 'kovats') and from what information to look for. It is recommended that the user provide a InChiKey or CAS number for each compound for they are much more reliable and non-ambiguous to look for in the NIST system in order to recover de retention index information.If more than one retention index is present, the the software returns the RI as a mean of the registered RIs.


```{r, out.width='40%', fig.align='center', echo=FALSE, fig.cap='Figure 32. Retention index information.'}
...
```{r, echo=FALSE, results='asis', fig.cap='Table 4. DatabaseInfo.csv table.'}
knitr::kable(read.csv(system.file("extdata", "DatabaseInfo.csv", package = "PipMet")), caption = 'Table 4. DatabaseInfo.csv table.')
```

And the creation of the in-house database is concluded (Figure 33).


```{r, out.width='40%', fig.align='center', echo=FALSE, fig.cap='Figure 33. In-house database created.'}
