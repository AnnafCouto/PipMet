...
Then the user will be asked to point the folder where the files to be processed are located. Once we set `example = TRUE`, this pop-up will be supressed.

```{r, out.width='40%', fig.align='center', fig.cap='Choose the samples folder.', echo=FALSE}
knitr::include_graphics('1.png')
``` 

Following that, the user is asked to provide a name for the processing project, which will be used as the name of the folder where the library will be processed occur and the output files created. For the example, the project will be named 'Example'.
...
The metadata file must be provided in order to proper process the analysis files. It is a sheet .csv file containing a 'cmpound' column for the analytes names, 'rt' for the retention time in seconds, 'formula', 'exact.mass', 'file' with the file path to each of the samples and some describers such as 'CAS', 'InChiKey', 'ChemSpider ID' and 'PubChem ID'. Each row represent a compound. In the set of examples, we have 6 compounds, with its multiple derivative molecular forms. The metadata.csv file will be as follow:


```{r, echo=FALSE, results='asis', fig.cap='Library metadata table.'}
knitr::kable(read.csv(system.file("extdata", "lib_metadata.csv", package = "PipMet")))
```

The PipMet asks if the user already have a metadata.csv file prepared, if the `example` is not set as TRUE. If 'Yes', the user will be asked to point the path to the metadata file. Though, if 'No', a lib_metadata.csv file will be created with basic names for files with the extension given in the sample_dir so the user can fill it.
...
When the spectra are finalized, it will be written in a  .msp file for each of the compound, to be uploaded in the NIST MS Search Software ^2^ the spectra manually validation with spectral databases.

```{r, out.width='35%', fig.align='center', fig.cap='Equipment information.', echo=FALSE}
knitr::include_graphics('c.png')
``` 

In this step, the user must upload the .msp file for the compound shown in the pop-up and select the id of the spectrum that correspond to that compound, sequentially. If there is a problem, the user may restart the validation step by choosing to 'Repeat' at the end.

```{r, out.width='35%', fig.align='center', fig.cap='Cellobiose (isomer 1) identification.', echo=FALSE}
knitr::include_graphics('d.png')
``` 

After the validation, a final .msp spectra is written, named after the name choosen in the beggining with the relevant information provided by the user in the library metadata table. To upload the .msp file as a library to NIST MS Search, the user may use the conversion tool LIB2NIST ^1^.
